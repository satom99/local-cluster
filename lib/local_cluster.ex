defmodule LocalCluster do
  ...
  def start_nodes(prefix, amount, options \\ [])
    ...
    nodes =
      Enum.map(1..amount, fn idx ->
        ...
        # Register the peer process locally on each node so that we can stop it later
        persist_node_peer(node, peer)

        node
      end)

    rpc = &:erpc.multicall(nodes, &1, &2, &3)
  ...
  defp persist_node_peer(node, peer) do
    :persistent_term.put({__MODULE__, node}, peer)
  end

  defp pop_node_peer(node) do
    key = {__MODULE__, node}

    peer = :persistent_term.get(key)
